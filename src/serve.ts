...
/** Wip */
export type ServeStaticHandler = (req: Request) => Response | Promise<Response>;
...
function evaluatePreconds(
  req: Request,
  preconds: PrecondHeaders | undefined,
):
  | Status.OK
...
function createContentTypeFromPathname(
  ...
  if (mime) {
    return mime.isUtf8 ? mime.type + `; charset=UTF-8` : mime.type;
  }
...
export const serveStatic: ServeStaticFactory =
  ({ rewrite = (x) => x } = {}) => async (req) => {
    ...
    const computeds = computeRangeHeader(
      req.headers.get(HeaderName.RANGE)!,
      size,
    );
    ...
    headers.set(
      HeaderName.CONTENT_RANGE,
      `${AcceptRangeUnit.BYTES} ${start}-${end}/${size}`,
    );
